<template>
  <el-drawer title="订单详情" v-model="dialogVisible" size="50%">
    ...
    <el-card v-if="info.ship_data" shadow="never" :body-style="{ padding: '20px' }" class="mb-3">
      <template #header>
    ...
    <el-card shadow="never" :body-style="{ padding: '20px' }" v-if="info.address" class="mb-3">
      <template #header>
    ...
    <el-card shadow="never" :body-style="{ padding: '20px' }" v-if="info.refund_status !== 'pending'" class="mb-3">
      <template #header>
...
const ship_time = computed(() => {
  if (props.info.ship_data) {
    const timeStr = useDateFormat(props.info.ship_data.express_time * 1000, 'YYYY-MM-DD HH:mm:ss')
